...
The three location-based searches: nearby by distance, by postal code, and by exact address. All three reuse `perf-avail` (from Decision 14).

**Decision 1 - distance in km via the standard Haversine formula, with the argument clamped to [-1, 1].**
...
**Decision 3 - ranking mode is chosen in Java, not passed as raw SQL**
...
**Decision 4 - a sold-out (or not-yet-priced) performance sorts last, in either price direction.**
...
**Decision 5 - adjacent postal code is one with the same first 2 characters (also covers same postal code)**
...
**Decision 6 - Q3 is an exact string match on `venue.address`**
...
**Decision 7 - one unified query, written as `reserved-half UNION ALL GA-half`.**
...
**Decision 8 - sold and available are current/active, not historical.**
...
**Decision 9 - "consecutive" means strict integer-consecutive available seats in one row.**
Seats n, n+1, ..., n+q-1, same row, all available (not SOLD, not
...
**Decision 10 - pricing shortcut: rank by cheapest qualifying row.** Decided that every seat in a row shares one price (a row sits in one section, a section maps to one tier for the performance). So any q consecutive seats in a row cost `q * price` - the total depends only on which row, not which block within it. "Lowest total" therefore collapses to "cheapest qualifying row," so Q7 prices rows, not every possible block. To be explained in later reports.

**Decision 11 - output is one summary row; ties broken deterministically.**
```
...
**Decision 12 - technique: gaps-and-islands with a window function.**
...
**Decision 13 - budget is optional, and it caps the whole block, not each seat.**
...
**Decision 14 - both lean on one shared "availability" summary per performance.**
...
**Decision 16 - what the filters actually mean.**
...
**Decision 17 - Q4 is Q5's date-and-availability filters bolted onto a location search.**
...
**Decision 18 - who's allowed is checked inside the statement, not before it.**
...
**Decision 19 - one operation, one transaction.**
...
**Decision 20 - the create-and-price flow, done in incremental steps.**
...
**Decision 21 - updating a tier's price, and when we refuse.**
...
**Decision 22 - blocking and unblocking a seat.**
...
**Decision 23 - cancelling a performance, the full cascade.**
